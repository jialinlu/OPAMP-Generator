...
if sim == True:
    ...
    if match1:
        ...
        prb_constr0 = - (gain - 85) / 85
        prb_constr1 = - (pm - 55) / 55
        prb_constr2 = - (gbw - 7e5) / 7e5
        prb_constr3 = (power - 250) / 250
        goal = -(gbw / 1e6) * (10 * 1e3) / (power * 1e-3) / 1e6

        res.writelines(str(goal) + ' ' + str(prb_constr0) + ' ' + str(prb_constr1) + ' ' + str(prb_constr2) + ' ' + str(
            prb_constr3) + '\n')
        res.close()
    else:
        print('simulation error!')
        goal = 1  # (np.random.rand()+1)*100
        prb_constr0 = 1  # (np.random.rand()+1)*-10
        prb_constr1 = 1  # (np.random.rand()+1)*-10
        prb_constr2 = 1  # (np.random.rand()+1)*0.2
        prb_constr3 = 1  # (np.random.rand()+1)*50
        res.writelines(str(goal) + ' ' + str(prb_constr0) + ' ' + str(prb_constr1) + ' ' + str(prb_constr2) + ' ' + str(
            prb_constr3) + '\n')
        res.close()
else:
